				...
				<div class="offset-md-2 col-md-4">
					<div class="card" style="width:200%;">
						<div class="card-header text-white" style="background: #9fd6ba;">
							<h2><span style="color: gray;"></span>회원 정보 수정</h2>
						</div>
						<div class="card-body">
							<form action="/userInfoMod" name="modify" id="ModForm" method="post"
								...
								<table
									...
									<c:if test="${loginUser.auth eq 'common'">
										<tr>
											...
											<input
												type="button" value="사업자 전환" 
												class="btn form-control tooltipstered" id="change-btn"
												style="background: ; margin-top: 0; height: 40px; color: white; border: 0px solid #388E3C; opacity: 0.8">
											</td>
											...
											<c:if test="">
												...
												<!-- 사업자 검증 모달창 띄우기 -->
												<!-- jQuery로 사업자 검증이가능 하도록 chk7, chk8 에 맞게 쓰자.   -->
												...
												<!-- 모달창에서 사업자 검증이 끝나면 -->
												<c:if test="">
													<input type="hidden" name="auth" value="business" id="auth">
												</c:if>
												<c:otherwise>
													<input type="hidden" name="auth" value="common" id="auth">
												</c:otherwise>
												<input type="hidden" name="auth" value="common" id="auth">
									...
									<c:if test="${loginUser.auth eq 'business'">
										<tr>
											...
											<!-- 일반사용자 전환 버튼이 눌리면 -->
											<!-- 버튼이 눌리면 businessNum과 buildingAddress를 null로 되돌린다. -->
...
$(function() {
	...
	//회원가입 form DOM객체 저장.
	const regForm = $("#ModForm");
	
	
	//auth 확인해서 common이라면 chk7과 chk8을 true로 바꿔준다. 
